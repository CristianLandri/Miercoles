...
DB_CONFIG = {
    ...
    'user': 'root',            
    'password': '',            
    'database': 'tienda_macetas'
}

PORT = 8081  # Puerto diferente para el servidor de macetas


def obtener_productos():
    conn = mysql.connector.connect(**DB_CONFIG)
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT * FROM macetas")  # Cambiado a tabla macetas
...
def generar_html():
    ...
    for p in productos:
        ...
        <div class="item">
            <figure>
                <img src="{p['imagen_url']}" alt="maceta">
            </figure>
    ...
    <head>
        ...
        <title>Tienda de Macetas</title>
        <link rel="stylesheet" href="css/tienda.css">
    </head>
    <body>
        <header>
            <h1>AgroSt❀r Macetas</h1>
        </header>
...
class MacetasHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/" or self.path == "/index.html":
            ...
            if id_producto:
                ...
                self.send_error(400, "ID de maceta requerido")
        else:
            super().do_GET()
    ...
    def do_POST(self):
        if self.path.startswith("/editar"):
            ...
            if id_producto and nombre and precio and imagen_url:
                ...
                self.send_error(400, "Faltan datos para actualizar la maceta")
        else:
    ...
    def mostrar_formulario_edicion(self, id_producto):
        conn = mysql.connector.connect(**DB_CONFIG)
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM macetas WHERE id = %s", (id_producto,))
        producto = cursor.fetchone()
        conn.close()
        if not producto:
            self.send_error(404, "Maceta no encontrada")
            return
        html = f"""
        <!DOCTYPE html>
        <html lang='es'>
    ...
    def actualizar_producto(self, id_producto, nombre, descripcion, precio, imagen_url):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE macetas SET nombre=%s, descripcion=%s, precio=%s, imagen_url=%s WHERE id=%s",
...
if __name__ == "__main__":
    servidor = HTTPServer(("localhost", PORT), MacetasHandler)
    print(f"✅ Servidor de Macetas activo en: http://localhost:{PORT}")
